...
export default function WhatsAppWebhookTab({
  ...
  const handleSetCurrentOriginWebhook = async () => {
    ...
    if (jwtToken) {
      try {
        await apiFetch('/api/admin/whatsapp/config', {
          method: 'POST',
  ...
  const handleSimulateDlr = async (e) => {
    ...
    try {
      const targetPhone = simDlrPhone.trim().replace(/[^0-9]/g, '') || (logs[0]?.recipient?.replace(/[^0-9]/g, '') || '60123456789');
      ...
      const res = await fetch(`/api/webhooks/whatsapp/dlr${dlrToken ? `?token=${encodeURIComponent(dlrToken)}` : ''}`, {
        method: 'POST',
          ...
          <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <div style={{ background: '#0F172A', color: '#38BDF8', padding: '10px', borderRadius: '4px', fontFamily: 'var(--font-code)', fontSize: '11px', overflowX: 'auto' }}>
              <pre style={{ margin: 0 }}>{JSON.stringify(simDlrResult?.payload || {
                ...
                status: simDlrStatus,
                "error-code": parseInt(simDlrErrorCode) || 0,
                recipient: simDlrPhone || '+60123456789',
                channel: 'whatsapp',
                cost: parseFloat(config.ratePerOtp) || 0.0075,
                delivered_at: new Date().toISOString()
              }, null, 2)}</pre>
            ...
            {simDlrResult?.response && (
              <div>
                ...
                <div style={{ background: 'var(--bg-ribbon)', border: '1px solid var(--border-subtle)', padding: '6px 10px', borderRadius: '4px', fontFamily: 'var(--font-code)', fontSize: '10px', color: '#059669', fontWeight: '700' }}>
